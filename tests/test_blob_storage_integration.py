...
class DocumentWithBlobs(DynamoDbVersionedResource):
    ...
    resource_config = ResourceConfig(
        ...
        blob_fields={
            ...
            "attachments": BlobFieldConfig(compress=True, content_type="application/json"),
        },
    )
...
class SimpleDocumentWithBlob(DynamoDbResource):
    ...
    resource_config = ResourceConfig(
        compress_data=False, blob_fields={"data": BlobFieldConfig(compress=False, content_type="text/plain")}
    )
...
class TestBlobStorageIntegrationWithMinIO:
    ...
    def test_create_and_retrieve_blob_fields(self, dynamodb_memory_with_s3):
        ...
        attachments_data = {"files": ["doc1.pdf", "doc2.xlsx"], "metadata": {"size": 102400, "type": "documents"}}
        ...
        # Load with blobs
        loaded_with_blobs = memory.get_existing(doc.resource_id, DocumentWithBlobs, load_blobs=True)
    ...
    def test_blob_version_preservation(self, dynamodb_memory_with_s3):
        ...
        loaded = memory.get_existing(doc.resource_id, DocumentWithBlobs, load_blobs=False)
        updated = memory.update_existing(loaded, {"title": "Version Test - Updated", "tags": ["v1", "v2"]})
        ...
        updated2 = memory.update_existing(updated, {"content": "Version 3 content"})

        assert updated2.version == 3

        # Load v3 and check blobs
        v3_loaded = memory.get_existing(doc.resource_id, DocumentWithBlobs, version=3, load_blobs=True)
    ...
    def test_blob_deletion(self, dynamodb_memory_with_s3, minio_s3_client):
        ...
        response = minio_s3_client.list_objects_v2(Bucket=memory.s3_bucket, Prefix=prefix)
    ...
    def test_non_versioned_resource_with_blobs(self, dynamodb_memory_with_s3):
        ...
        doc = memory.create_new(SimpleDocumentWithBlob, {"name": "Simple Test", "data": large_data})
        ...
        # Load without blob
        loaded = memory.get_existing(doc.resource_id, SimpleDocumentWithBlob, load_blobs=False)

        assert loaded.data is None
        assert loaded.has_unloaded_blobs()

        # Load with blob
        loaded_with_blob = memory.get_existing(doc.resource_id, SimpleDocumentWithBlob, load_blobs=True)
    ...
    def test_clear_blob_field(self, dynamodb_memory_with_s3):
        ...
        # Clear content field
        updated = memory.update_existing(doc, update_obj={}, clear_fields={"content"})
    ...
    def test_s3_key_structure(self, dynamodb_memory_with_s3, minio_s3_client):
        ...
        doc = memory.create_new(
            DocumentWithBlobs,
            {"title": "S3 Key Test", "author": "Tester", "tags": ["s3"], "content": "Test content for S3"},
        )
        ...
        simple_doc = memory.create_new(SimpleDocumentWithBlob, {"name": "Simple S3 Test", "data": "Simple data"})
