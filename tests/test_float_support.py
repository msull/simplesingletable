...
def test_basic_float_support(dynamodb_memory: DynamoDbMemory):
    ...
    retrieved = dynamodb_memory.read_existing(
...
def test_versioned_resource_float_updates(dynamodb_memory: DynamoDbMemory):
    ...
    v2 = dynamodb_memory.update_existing(
        resource,
        {
            "price": 45.99,
            "rating": 4.0,
    ...
    # Create version 3
    v3 = dynamodb_memory.update_existing(
        v2,
        {
            "price": 39.99,
            "rating": 4.5,
            "optional_weight": 0.75
        }
    )
...
def test_null_and_optional_floats(dynamodb_memory: DynamoDbMemory):
    ...
    with_discount = dynamodb_memory.update_existing(
        resource,
        {"discount": 0.10}
    )
    ...
    no_discount = dynamodb_memory.update_existing(
        with_discount,
        {"discount": None}
    )
...
def test_float_clear_fields(dynamodb_memory: DynamoDbMemory):
    ...
    cleared = dynamodb_memory.update_existing(
        resource,
        {"price": 21.99},
        clear_fields={"discount"}
    )
...
def test_float_in_gsi_operations(dynamodb_memory: DynamoDbMemory):
    ...
    all_products = dynamodb_memory.list_type_by_updated_at(
        ComplexFloatResource,
        ascending=True
    )
...
def test_uncompressed_vs_compressed_float_behavior(dynamodb_memory: DynamoDbMemory):
    ...
    retrieved_uncompressed = dynamodb_memory.read_existing(
        uncompressed.resource_id,
        UncompressedFloatResource
    )
    ...
    retrieved_compressed = dynamodb_memory.read_existing(
        compressed.resource_id,
        ComplexFloatResource
    )
